  ...
  const response = await fetch(`${GROQ_API}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "Ты безопасный coding worker портфолио KILA. Верни только JSON: {\"files\":[{\"path\":\"...\",\"content\":\"полный новый текст файла\"}],\"summary\":\"краткий итог\"}. Изменяй минимум файлов. Используй только переданные файлы. Не меняй API, webhook, секреты, зависимости и инфраструктуру. Не удаляй файлы.",
        },
...
async function pushChanges(files, task) {
  ...
  const commit = await github(`/repos/${repo}/git/commits`, {
    method: "POST",
    body: JSON.stringify({ message: `Telegram task: ${task.replace(/\s+/g, " ").slice(0, 72)}`, tree: tree.sha, parents: [parentSha] }),
  });
