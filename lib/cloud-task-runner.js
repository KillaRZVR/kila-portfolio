...
async function github(path, options = {}) {
  const response = await fetch(`${GITHUB_API}${path}`, {
    ...options,
    headers: { Authorization: `Bearer ${env("GITHUB_TOKEN")}`, Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "Content-Type": "application/json", ...(options.headers || {}) },
  });
...
function extension(path) { const index = path.lastIndexOf("."); return index >= 0 ? path.slice(index).toLowerCase() : ""; }
function isEditablePath(path, existingPaths) {
  return existingPaths.has(path) && EDITABLE_PREFIXES.some((prefix) => path.startsWith(prefix)) && !BLOCKED_PREFIXES.some((prefix) => path.startsWith(prefix)) && TEXT_EXTENSIONS.has(extension(path));
}
function selectRelevantPaths(task) {
  ...
  if (has("пример работ", "три направления", "накладыва", "карточ", "проект")) add("components/projects-section.tsx");
  if (has("независим", "логотип", "килла", "kila", "шапк", "хедер")) add("components/header.tsx");
  if (has("главн", "первый экран", "компакт", "подвин")) add("app/page.tsx");
  if (has("куб", "анимац")) add("components/hero-artifact.tsx");
  if (has("пример работ", "цен", "обратн", "навигац")) add("components/hero-buttons.tsx");
  if (has("шрифт", "цвет", "фон", "тем")) add("app/globals.css");
  if (has("цен", "стоим", "контакт", "email", "телеграм", "telegram")) add("components/price-section.tsx");
  if (has("подвал", "footer", "права")) add("components/footer.tsx");
  if (has("текст", "подпись", "названи", "контент")) add("lib/data.js");
...
async function loadRepositoryContext(task) {
  ...
  const available = new Map(tree.tree.filter((entry) => entry.type === "blob" && entry.size <= 80000).filter((entry) => EDITABLE_PREFIXES.some((prefix) => entry.path.startsWith(prefix))).filter((entry) => !BLOCKED_PREFIXES.some((prefix) => entry.path.startsWith(prefix))).filter((entry) => TEXT_EXTENSIONS.has(extension(entry.path))).map((entry) => [entry.path, entry]));
...
async function planChanges(task, files) {
  ...
  const response = await fetch(`${GROQ_API}/chat/completions`, {
    ...
    body: JSON.stringify({
      model: env("GROQ_CODE_MODEL", "llama-3.3-70b-versatile"), temperature: 0.1, max_tokens: 3000, response_format: { type: "json_object" },
      messages: [
...
function normalizeGeneratedPath(value) {
  return String(value || "").trim().replace(/^`+|`+$/g, "").replace(/\\/g, "/").replace(/^\.\//, "").replace(/^\//, "").replace(/^src\//i, "");
}
function validateChanges(result, files) {
  const originals = new Map(files.map((file) => [file.path, file.content]));
  const pathLookup = new Map(files.map((file) => [file.path.toLowerCase(), file.path]));
  ...
  for (const file of result.files) {
    ...
    const path = pathLookup.get(normalizedPath.toLowerCase()) || normalizedPath;
