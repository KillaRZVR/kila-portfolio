...
export function ProjectsSection() {
  ...
  return (
    <section id="projects" className="relative scroll-mt-12 py-14 sm:py-20">
      <Separator className="opacity-40" />
      <RevealSection className="relative pt-6">
        <span className="pointer-events-none absolute left-0 top-2 text-[clamp(5rem,11vw,9rem)] leading-none tracking-[-0.1em] text-foreground/[0.045]">{siteData.projects.sectionNumber}</span>
        <div className="relative mx-auto mb-10 max-w-none text-center sm:mb-12">
          <h2 className="display-tight whitespace-nowrap text-[clamp(3rem,7.5vw,7.2rem)] font-normal">{siteData.projects.title}</h2>
          <p className="font-telemetry mt-4 text-[11px] uppercase text-muted-foreground">{siteData.projects.hint}</p>
        </div>
              ...
              <motion.div key={project.id} layout>
                <Card role="button" tabIndex={0} aria-expanded={isOpen} onClick={() => setOpenId(isOpen ? null : project.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setOpenId(isOpen ? null : project.id); } }} className="group cursor-pointer overflow-hidden rounded-none border-0 bg-transparent shadow-none focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#f5c2c8]">
                  <div className="aspect-[4/3] bg-[#1b1c18] p-3 transition-colors duration-500 group-hover:bg-[#20211d]">
                    <span className="font-telemetry text-[10px] text-muted-foreground">0{index + 1} / 03</span>
                  ...
                  <CardHeader className="flex-row items-start justify-between space-y-0 px-0 py-4">
                    <div className="pr-4">
                      <h3 className="text-[clamp(1.6rem,2.5vw,2.4rem)] font-normal tracking-[-0.04em]">{project.title}</h3>
                      <p className="font-telemetry mt-1 text-[10px] uppercase text-[#f5c2c8]">{project.category}</p>
