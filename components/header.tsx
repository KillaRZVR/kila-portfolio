    ...
    <header id="top" className="relative z-40 px-4 pt-4 sm:px-6 sm:pt-5 lg:px-8">
      <div className="flex items-start justify-between gap-3 sm:gap-6">
        <a href="#top" className="font-xanmono block w-[58vw] sm:w-[38vw] md:relative md:pb-6 lg:w-[28vw] lg:max-w-[32rem]" aria-label="На главную">
          ...
          <motion.span initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45, duration: 0.45 }} className="font-xanmono mt-1 block max-w-64 text-[10px] uppercase leading-tight text-muted-foreground md:absolute md:left-full md:top-1/2 md:ml-4 md:mt-0 md:w-64 md:-translate-y-1/2 md:text-xs lg:ml-6">{siteData.eyebrow}</motion.span>
        </a>
